...
class MultiResUnet_Modified(torch.nn.Module):
    ...
    def forward(self,x : torch.Tensor)->torch.Tensor:
        ...
        up6_sum = torch.cat([up6_sum,torch.zeros(up6_sum.shape).to(device=DEVICE,dtype=torch.float)],axis=1)
        up6 = up6 + up6_sum
        ...
        up7_sum = torch.cat([up7_sum,torch.zeros(up7_sum.shape).to(device=DEVICE,dtype=torch.float)],axis=1)
        up7 = up7 + up7_sum
        ...
        up8_sum = torch.cat([up8_sum,torch.zeros(up8_sum.shape).to(device=DEVICE,dtype=torch.float)],axis=1)
        up8 = up8 + up8_sum
        ...
        up9_sum = torch.cat([up9_sum,torch.zeros(up9_sum.shape).to(device=DEVICE,dtype=torch.float)],axis=1)
        up9 = up9 + up9_sum
